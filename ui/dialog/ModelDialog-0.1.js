...
ModelDialog = function() {
    ...
    ModelDialog.prototype = {
        init: function() {
            ...
            this.dialog = $('div').setCls(this.dialogCls).css('position:fixed;z-index:100;')[0];
            ...
            docBody.appendChild(this.dialog);
            this.moveToCenter();
            this.addEvent()
        },
        destroy: function() {
        ...
        createShadow: function() {
            ...
            var str = 'position:fixed;left:0px;top:0px;z-index:1' + 
                ';width:' + size.width + 'px' +
        ...
        addEvent: function() {
            ...
            this.on(window,'resize', function() {
                var size = getViewSize()
                _this.shadow.style.width = size.width + 'px'
                _this.shadow.style.height = size.height + 'px'
                _this.moveToCenter()
            })
            if (ie6) {
                this.on(window, 'scroll', function() {
                    _this.shadow.style.top = document.documentElement.scrollTop + 'px'
                    _this.moveToCenter()
                })
            }
        },
        ...
        moveToCenter: function() {
            var size = getViewSize()
            var x = (size.width-50)/2 - (this.dialog.clientWidth-50)/2
            var y = (size.height-50)/2 - (this.dialog.clientHeight-50)/2
            if (ie6) {
        ...
        onClose: function() {
            var docBody = document.body
            docBody.removeChild(this.dialog);
            docBody.removeChild(this.shadow);
            this.destroy();     
        },
