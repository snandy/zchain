function ProvinceCity(selectProvince, selectCity){
	// 省市联动数据
	this.data = {
		'请选择' : ['---'],
	...
	this.province = selectProvince;
	this.city = selectCity;
	this.init(selectProvince,selectCity);
}
ProvinceCity.prototype = {
	init : function(select1,select2){
		var data = this.data;
		for(var n in data){		
		...
		select1.onchange = function(){
			select2.innerHTML = '';
			var ary = data[this.value];
			for(var i=0,len=ary.length;i<len;i++){
				var opt = document.createElement('option');
				opt.value = opt.innerHTML = ary[i];
